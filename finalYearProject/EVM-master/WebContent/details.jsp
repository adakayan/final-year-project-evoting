...
<%} %>
 <div id="whole">
    <section id="sec">
    <a href="schedule.jsp">Add New Election</a>
    <center><b><p style="font-family: Arial, Helvetica, sans-serif;font-size: 40px;color:red;">Election Details</p></b></center>
    	<div id="cont">
    	<table border="1">
    	<tr><td><b>ElectionDate</b></td><td><b>Constituency</b></td><td><b>Countingdate</b></td><td><b>Election Id</b></td><td><b>Voter Count</b></td></tr>
    	<%
    	String clas=request.getParameter("clas");
    	ResultSet res=ob.viewschedule(clas);
    	while(res.next()){
    	String id=res.getString("electionid");%>
    	<tr><form method="post">
    		<td><%=res.getString("Electiondate")%></td><td><%=res.getString("Constituency") %></td><td><%=res.getString("Countingdate") %></td><td><%=res.getString("electionid") %></td><td><%=res.getString("votercount") %></td><td><a href="editelection.jsp?id=<%=res.getString("electionid") %>">Edit</a></td><td><a href="deleteelection.jsp?id=<%=res.getString("electionid") %>">Delete</a></td>
        </form></tr>
    	<%}
    	%>
    	</table></div>
